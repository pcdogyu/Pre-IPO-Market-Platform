...
func TestAdminCanUpdateDealStatus(t *testing.T) {
	...
	if err := s.UpdateDealStatus(context.Background(), admin.ID, 1, "cancelled", "发行方暂停分配"); err != nil {
	...
	if err := s.UpdateDealStatus(context.Background(), admin.ID, 1, "open", "重新开放已取消项目"); err == nil {
...
func TestPostInvestmentAndOpsWorkflows(t *testing.T) {
	...
	if err := s.CreateReport(context.Background(), admin.ID, domain.InvestorReport{UserID: 2, ReportType: "portfolio", Title: "二季度组合报告", Period: "2026-Q2", Status: "available"}); err != nil {
		t.Fatalf("create report: %v", err)
	}
	if err := s.CreateReport(context.Background(), admin.ID, domain.InvestorReport{UserID: 2, ReportType: "tax", Title: "2026年税务草稿", Period: "2026", Status: "pending"}); err != nil {
	...
	var reportID int64
	for _, report := range reports {
		if report.Title == "2026年税务草稿" && report.UserName != "" && report.Status == "pending" {
			reportID = report.ID
			break
		}
	}
	...
	if err := s.CreateRiskAlert(context.Background(), admin.ID, domain.RiskAlert{Severity: "high", Status: "open", Subject: "集中度限制", Note: "复核敞口"}); err != nil {
	...
	if err := s.AddRiskAction(context.Background(), admin.ID, alerts[0].ID, admin.ID, "assigned", "负责人已分配"); err != nil {
...
func TestNegotiationWorkflows(t *testing.T) {
	...
	if err := s.CreateNegotiation(context.Background(), admin, 1, 42, 800, "管理员备注"); err != nil {
...
func TestSubscriptionDocumentWorkflow(t *testing.T) {
	...
	if err := s.CreateSubscriptionDocument(context.Background(), admin.ID, 1, "Risk Disclosure", "风险文件包"); err != nil {
